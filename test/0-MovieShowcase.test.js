...
describe("<MovieShowcase />", () => {
  ...
  it("has generated the correct prop values for the first movie", () => {
    ...
    expect(JSON.stringify(cards.at(0).props().genres)).to.equal(
      JSON.stringify(["cats", "adventure", "romance"])
    );
  ...
  it("has generated the correct prop values for the last movie", () => {
    ...
    expect(JSON.stringify(cards.last().props().genres)).to.equal(
      JSON.stringify(["crime", "period piece"])
    );
